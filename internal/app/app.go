...
// that every feature component can accept an App without importing the cmd
// package, which would create an import cycle (cmd imports the components, and
// the components would import cmd). The cmd layer constructs the App in
// cmd/root.go and passes it down to each component.
package app
...
// in cmd/root.go after configuration resolution and prerequisite checks
// succeed.
type App struct {
	// Config is the effective configuration resolved for this invocation (flag >
	// env > persisted > default).
	Config config.Config
	// GitHub performs GitHub API operations (resolve repositories, create forks).
	GitHub githubclient.GitHubClient
	// Git executes local git commands under the workspace root.
	Git git.Runner
