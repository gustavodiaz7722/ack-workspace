...
// newAddCommand builds the `add` subcommand, which forks, clones, and
// configures one or more service controller repositories named by the supplied
// service identifiers.
//
// add performs git and GitHub API operations and needs the contributor's
// identity, so it declares all three prerequisites, enforced before the adder
// runs.
//
// The empty-identifier case is intentionally NOT enforced via cobra Args.
// Delegating to the adder, which returns an
...
// of RunE so the entrypoint can map it to a usage exit code.
func newAddCommand(d deps, res *Result) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// args are the service identifiers. An empty list is rejected by the adder
			// rather than by cobra so the rule lives in one place.
